...
import { cn, hoursFromMinutes } from "@/lib/utils";
...
export default async function ReportDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const ruleEvaluation = (eod?.rule_evaluation ?? null) as
    | {
        passed?: Array<{ code: string; title: string; expected: string; actual: string; deviation: string; scoreImpact: number }>;
        violated?: Array<{ code: string; title: string; expected: string; actual: string; deviation: string; scoreImpact: number }>;
        context?: { ruleSetName: string; scope: "GLOBAL" | "DEPARTMENT"; department: string | null; roleRuleApplied: boolean };
      }
    | null;
  const passedRules = Array.isArray(ruleEvaluation?.passed) ? ruleEvaluation?.passed : [];
  const violatedRules = Array.isArray(ruleEvaluation?.violated) ? ruleEvaluation?.violated : [];
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>5. HR Rule Engine Result</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="text-sm text-muted-foreground">
            Rule set: {ruleEvaluation?.context?.ruleSetName ?? "N/A"} ({ruleEvaluation?.context?.scope ?? "N/A"})
            {ruleEvaluation?.context?.department ? ` - ${ruleEvaluation.context.department}` : ""}
          </div>
          <section className="grid gap-4 md:grid-cols-3">
            <MetricCard icon={CheckCircle2} label="Rules Passed" value={String(passedRules.length)} note="Configured thresholds met" />
            <MetricCard icon={TriangleAlert} label="Rules Violated" value={String(violatedRules.length)} note="Threshold deviations detected" />
            <MetricCard icon={Gauge} label="Score Impact" value={String(violatedRules.reduce((sum, item) => sum + Math.min(0, Number(item.scoreImpact ?? 0)), 0))} note="Total negative impact" />
          </section>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[980px] text-left text-sm">
              <thead className="text-xs uppercase text-muted-foreground">
                <tr>
                  {["Rule", "Expected", "Actual", "Deviation", "Score Impact", "Status"].map((head) => (
              ...
              <tbody>
                {[...violatedRules, ...passedRules].map((rule) => {
                  const passed = !violatedRules.some((item) => item.code === rule.code);
                  return (
                    <tr key={rule.code} className="hover:bg-muted/40">
                      <td className="px-3 py-3">{rule.title}</td>
                      <td className="px-3 py-3">{rule.expected}</td>
                      <td className="px-3 py-3">{rule.actual}</td>
                      <td className="px-3 py-3">{rule.deviation}</td>
                      <td className={cn("px-3 py-3", Number(rule.scoreImpact) < 0 ? "text-red-300" : "text-emerald-300")}>{Number(rule.scoreImpact) > 0 ? `+${rule.scoreImpact}` : rule.scoreImpact}</td>
                      <td className="px-3 py-3">{passed ? "Passed" : "Violated"}</td>
                    </tr>
                  );
                })}
                {!passedRules.length && !violatedRules.length ? (
                  <tr>
                    <td className="px-3 py-3 text-muted-foreground" colSpan={6}>
                      No rule evaluation payload found for this report.
                    </td>
                  </tr>
                ) : null}
              </tbody>
